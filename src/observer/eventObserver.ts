...
> {
  ...
  private observeScroll() {
    ...
    const timerList = new Map<EventTarget | null, number>();
      ...
      (e: UIEvent) => {
        if (!symbolList.has(e.target)) {
          ...
          timerList.set(
            e.target,
            window.setTimeout(() => {
              symbolList.delete(e.target);
              clearTimeout(timerList.get(e.target));
              timerList.delete(e.target);
            }, 5000),
          );
        } else if (timerList.has(e.target)) {
          clearTimeout(timerList.get(e.target));
          timerList.delete(e.target);
          timerList.set(
            e.target,
